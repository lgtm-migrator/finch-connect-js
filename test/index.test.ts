  ...
  describe('initialize', () => {
    ...
    test('sets up an iframe with a valid auth url', () => {
      ...
      expect(iframe?.src).toEqual(
        `https://connect.tryfinch.com/authorize?client_id=client-id&products=company+directory+employment&app_type=spa&redirect_uri=https%3A%2F%2Ftryfinch.com&mode=employer&sdk_version=js-1.1.0`
      );
    })
  })
